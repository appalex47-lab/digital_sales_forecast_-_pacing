...
(function (root) {
    ...
    $('rc-simulator').innerHTML = `
      ...
      <div class="filters rc-save">
        ...
        <div class="field"><span class="field__hint">&nbsp;</span><div class="btn-row">
          ...
          <button type="button" class="btn btn--quiet" data-action="rc-draft-reset">Limpiar</button></div></div>
      </div>`;
  }
  ...
  function renderSaved(state) {
    ...
    const rows = list.map((s) => `<tr>
      ...
      <td><button type="button" class="btn btn--small btn--quiet" data-action="rc-edit-scenario" data-id="${esc(s.scenarioId)}">Nueva versión</button></td></tr>`).join('');
