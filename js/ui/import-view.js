...
(function (root) {
  ...
  function renderCards(state) {
    ...
    $('mock-files').innerHTML = `
      ...
      <button type="button" class="btn" data-action="stage-all-generated">Revisar los cuatro datasets generados</button>`;
  }
  ...
  function renderStaging(state) {
    ...
    const tabs = items.length > 1 ? `<div class="file-tabs" role="tablist">${items.map((i) => {
      const bad = i.result.mappingIssues.length || i.result.summary.error;
      return `<button type="button" role="tab" aria-selected="${i === active}" data-action="select-staged" data-id="${i.staged.id}">
        ${esc(i.staged.fileName)} <span class="cell-sub">${esc(C().dataTypes[i.staged.dataType].label)}${bad ? ', con problemas' : ''}</span></button>`;
    }).join('')}</div>` : '';
    ...
    const mappingRows = staged.parsed.headers.map((h) => {
      ...
      return `<tr>
        ...
        <td class="wrap cell-sub">${esc(samples.join(', ') || '(vacía)')}</td>
