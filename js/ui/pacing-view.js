...
(function (root) {
      ...
      <div class="filters">
        ...
        <div class="field"><span class="field__hint">Atajos</span>
          <div class="btn-row">
            ...
            <button type="button" class="btn btn--small btn--quiet" data-action="fc-ref-today">Hoy</button>
          </div></div>
  ...
  function renderExecutive(state) {
    ...
    $('fc-kpis').innerHTML = `
      <div class="subhead"><h3 class="panel__title">Total digital · ${esc(C().metrics[k].label)}</h3>
        <span>${pacePill(a.pacingStatus && k === 'revenue' ? a.pacingStatus : FP.gap.pacingStatus(td.compliance, run.settings.pacingThresholds))}</span></div>
  ...
  function renderChart(state) {
    ...
    const all = [...s.plan, ...s.actual, ...s.forecast].filter((v) => v !== null && Number.isFinite(v));
    if (!all.length) { $('fc-chart').innerHTML = '<div class="empty"><strong>Sin datos para graficar</strong></div>'; return; }
    ...
    const fmtTick = (v) => (k === 'revenue' ? F().currencyCompact(v) : k === 'conversionRate' ? F().percent(v, 2) : k === 'aov' ? F().currency(v, 0) : F().integer(v));
    const monthTicks = [];
    days.forEach((d, i) => { if (d.date.slice(8) === '01') monthTicks.push([i, MONTHS[+d.date.slice(5, 7) - 1].slice(0, 3)]); });
    const cut = s.cutoffIndex >= 0 ? x(s.cutoffIndex) : null;
    ...
    $('fc-chart').innerHTML = `
      ...
      <ul class="legend">
        <li><span class="legend__swatch legend__swatch--plan"></span>Plan acumulado ${esc(val(k, last(s.plan)))}</li>
        <li><span class="legend__swatch legend__swatch--actual"></span>Actual acumulado ${esc(val(k, last(s.actual)))}</li>
        <li><span class="legend__swatch legend__swatch--forecast"></span>Forecast ${esc(val(k, last(s.forecast)))}</li>
      </ul>
      <div class="chart-wrap"><svg viewBox="0 0 ${W} ${Hh}" role="img" aria-label="Plan, actual y forecast acumulados de ${esc(chLabel(ch))}">
        ${ticks.map((t) => `<line x1="${L}" x2="${W - R}" y1="${y(t).toFixed(1)}" y2="${y(t).toFixed(1)}" class="chart-grid"/>
          <text x="${L - 8}" y="${(y(t) + 4).toFixed(1)}" class="chart-label" text-anchor="end">${esc(fmtTick(t))}</text>`).join('')}
        ...
        ${cut !== null ? `<line x1="${cut.toFixed(1)}" x2="${cut.toFixed(1)}" y1="${T}" y2="${Hh - B}" class="chart-cut"/>
          <text x="${(cut + 4).toFixed(1)}" y="${T + 10}" class="chart-label">corte ${esc(run.cutoff)}</text>` : ''}
        <path d="${path(s.plan)}" class="chart-line chart-line--plan"/>
        <path d="${path(s.forecast)}" class="chart-line chart-line--forecast"/>
        <path d="${path(s.actual)}" class="chart-line chart-line--actual"/>
      </svg></div>
