...
(function (root) {
  ...
  function renderTable(state) {
    ...
    box.innerHTML = `<div class="table-wrap"><table class="table">
      <thead><tr><th>Fecha</th><th>Canal</th><th>Tipo de día</th><th class="num">Venta</th><th class="num">Pedidos</th>
      ...
      <tbody>${slice.map((r) => {
        ...
        return `<tr>
          ...
          <td>${esc(DAY_TYPE[r.dayType] || r.dayType)}${extra ? `<span class="cell-sub">${esc(extra)}</span>` : ''}</td>
          ...
          <td class="cell-sub">${esc(r.provenance.fileName)}, fila ${r.provenance.row}</td>
        </tr>`;
