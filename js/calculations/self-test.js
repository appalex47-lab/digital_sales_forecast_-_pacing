...
(function (root) {
  ...
  function run() {
    ...
    if (FP.guidanceConfig && FP.guidanceConfig.METHOD_GUIDE && FP.explain) {
      ...
      test('Fase 9.1: preparación de datos', 'Nada cargado: 0 %; sin venta real, tope de 45 % aunque lo demás esté completo', () => {
        const base = { year: 2026, targets: { defined: false }, plan: { original: false, imported: false }, actual: { countedDays: 0 },
          data: { historical: 0, quality: 'empty' } };
        const none = CE.dataReadiness(base);
        const noActual = CE.dataReadiness({ year: 2026, targets: { defined: true }, plan: { original: true }, actual: { countedDays: 0 },
          data: { historical: 50, quality: 'ready' } });
        return none.percent === 0 && none.tier === 'error' && !none.allBlockingDone &&
          noActual.percent <= 45 && noActual.tier === 'error' && !noActual.allBlockingDone;
      });
      test('Fase 9.1: preparación de datos', 'Con todo cargado: 100 %, verde, y nota explícita de que no mide el negocio', () => {
        const full = { year: 2026, targets: { defined: true }, plan: { original: true, imported: false }, actual: { countedDays: 366 },
          data: { historical: 50, quality: 'ready' } };
        const r100 = CE.dataReadiness(full);
        return r100.percent === 100 && r100.tier === 'ok' && r100.allBlockingDone &&
          typeof r100.note === 'string' && /No mide el desempeño del negocio/.test(r100.note) &&
          r100.items.length === 6 && r100.items.filter((it) => it.blocking).length === 3 && r100.items.every((it) => it.done);
      });
      test('Fase 9.1: preparación de datos', 'El semáforo respeta sus propios cortes; el plan importado cuenta igual que el propio; más completo nunca baja el %', () => {
        const cases = [
          { year: 2026, targets: { defined: false }, plan: { original: false, imported: false }, actual: { countedDays: 0 }, data: { historical: 0, quality: 'empty' } },
          { year: 2026, targets: { defined: true }, plan: { original: false, imported: false }, actual: { countedDays: 0 }, data: { historical: 0, quality: 'empty' } },
          { year: 2026, targets: { defined: true }, plan: { original: true, imported: false }, actual: { countedDays: 10 }, data: { historical: 0, quality: 'invalid' } },
          { year: 2026, targets: { defined: true }, plan: { original: true, imported: false }, actual: { countedDays: 366 }, data: { historical: 50, quality: 'ready' } }
        ];
        const results = cases.map((c) => CE.dataReadiness(c));
        const tiersMatchCuts = results.every((r) => r.tier === (r.percent >= 85 ? 'ok' : r.percent >= 50 ? 'warning' : 'error'));
        const monotonic = results.every((r, i) => i === 0 || r.percent >= results[i - 1].percent);
        const reachesWarning = results.some((r) => r.tier === 'warning');
        const importedPlan = CE.dataReadiness({ year: 2026, targets: { defined: true }, plan: { original: false, imported: true }, actual: { countedDays: 366 },
          data: { historical: 5, quality: 'ready' } });
        return tiersMatchCuts && monotonic && reachesWarning &&
          importedPlan.allBlockingDone && importedPlan.items.find((i) => i.id === 'plan').done === true;
      });
